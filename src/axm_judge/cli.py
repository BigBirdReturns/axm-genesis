...
MAGIC_LAT_FILE = b"AXLF"   # file-level header, written once at open
MAGIC_LAT = b"AXLR"        # per-record magic for latent stream
MAGIC_RESID = b"AXRR"      # per-record magic for residual stream
FILE_HEADER_LEN = 4         # AXLF file magic, must be skipped before records
VERSION = 1
...
class StreamJudge:
    def __init__(self, capsule_path: Path):
        ...
        # Validate and skip 4-byte AXLF file header
        file_magic = self.f_lat.read(FILE_HEADER_LEN)
        if len(file_magic) < FILE_HEADER_LEN or file_magic != MAGIC_LAT_FILE:
            raise JudgeError(
                f"cam_latents.bin: bad file header "
                f"(expected {MAGIC_LAT_FILE!r}, got {file_magic!r})"
            )
