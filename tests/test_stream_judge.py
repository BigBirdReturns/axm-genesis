...
MAGIC_LAT_FILE = b"AXLF"  # file-level header
MAGIC_LAT = b"AXLR"    # per-record magic
MAGIC_RESID = b"AXRR"  # residual per-record magic
VERSION = 1
...
def _write_latents(path: Path, n_frames: int) -> None:
    with open(path, "wb") as f:
        f.write(MAGIC_LAT_FILE)  # AXLF file header (4 bytes, skipped before records)
...
def _write_events(path: Path, n_frames: int, *, offset_override: dict[int, int] | None = None) -> None:
    ...
    with open(path, "wb") as f:
        for fid in range(n_frames):
            off = offset_override.get(fid, 4 + fid * LATENT_REC_LEN)  # 4 = AXLF file header
            evt = {
                "frame_id": fid,
                "stream_refs": {
...
def test_offset_drift_causes_hard_fail(tmp_path: Path) -> None:
    ...
    _write_events(capsule / "events.jsonl", n_frames=12, offset_override={7: (4 + 7 * LATENT_REC_LEN) + 1})  # 4 = AXLF file header
