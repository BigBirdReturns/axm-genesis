...
| [0002](0002-v1-reset.md) | The v1.0 Reset — Freeze Once, Freeze Right | **IMPLEMENTED** (pending ceremony + tag) | Hybrid suite `axm-hybrid1` (Ed25519 ‖ ML-DSA-44, both must verify); canonical JSONL core tables; ASCII-only lowercasing in `canonicalize()`; naming as proposed (`sh1_`, `e1_`, `c1_`, `p1_`, `s1_`). Spec/v1, kernel, vectors, provisional gold v2 landed; key ceremony and v1.0.0 tag remain (RELEASE.md) |
